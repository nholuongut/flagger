...
run:
	...
	-metrics-server=https://prometheus.istio.https://github.com/nholuongut \
...
run-nginx:
	...
	-metrics-server=http://prometheus-weave.istio.https://github.com/nholuongut
...
run-linkerd:
	...
	-metrics-server=https://linkerd-prometheus.istio.https://github.com/nholuongut

build:
	GIT_COMMIT=$$(git rev-list -1 HEAD) && GO111MODULE=on CGO_ENABLED=0 GOOS=linux go build  -ldflags "-s -w -X github.com/nholuongut/flagger/pkg/version.REVISION=$${GIT_COMMIT}" -a -installsuffix cgo -o ./bin/flagger ./cmd/flagger/*
	docker build -t nholuongut/flagger:$(TAG) . -f Dockerfile

push:
	docker tag nholuongut/flagger:$(TAG) nholuongut/flagger:$(VERSION)
	docker push nholuongut/flagger:$(VERSION)
...
helm-package:
	...
	mv charts/*.tgz bin/
	curl -s https://raw.githubusercontent.com/nholuongut/flagger/gh-pages/index.yaml > ./bin/index.yaml
...
dev-up: version-up
	...
	docker build -t quay.io/nholuongut/flagger:$(VERSION) . -f Dockerfile
	docker push quay.io/nholuongut/flagger:$(VERSION)
...
loadtester-run: loadtester-build
	docker build -t nholuongut/flagger-loadtester:$(LT_VERSION) . -f Dockerfile.loadtester
	docker rm -f tester || true
	docker run -dp 8888:9090 --name tester nholuongut/flagger-loadtester:$(LT_VERSION)
...
loadtester-push:
	docker build -t nholuongut/flagger-loadtester:$(LT_VERSION) . -f Dockerfile.loadtester
	docker push nholuongut/flagger-loadtester:$(LT_VERSION)
